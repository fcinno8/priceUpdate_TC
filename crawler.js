...
async function main() {
  try {
    ...
    const TC_LIST = await playerSearch([214], 0); // playerSearch(시즌넘버, 최소오버롤)
    let TC_RESULTS = await playerPriceValue(TC_LIST, [1, 2, 3, 4, 5, 6, 7, 8]); // playerPriceValue(데이터 , 강화등급)
    await saveToDB(TC_RESULTS);
